...
function buildCalendarHref({ weekStart, workerId, status }) {
...
export default async function CalendarPage({ searchParams }) {
  ...
  const weekParam =
    typeof sp?.week === "string" && isValidCalendarDateString(sp.week)
      ? sp.week
      : null;
  const { weekStart, weekEnd, days } = mondayWeekContaining(weekParam);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Week of {formatCalendarDate(weekStart)} · America/Chicago
            </p>
          </div>
