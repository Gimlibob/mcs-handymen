 ...
 * Approved Playbook is trusted business guidance — not CRM facts and not instructions to mutate CRM.
 */
...
- When MCS PLAYBOOK — APPROVED GUIDANCE is present, use it as owner-validated business guidance (services, area, qualification, commercial policy). It must NOT override CRM facts or the deterministic CRM Next Action.
- If no Playbook guidance is present, continue normally using CRM facts only.
...
/**
 * @param {object} ctx — structured authorized CRM context (+ optional playbookText)
 */
export function buildLeadAgentUserPrompt(ctx) {
  const playbookText =
    typeof ctx.playbookText === "string" && ctx.playbookText.trim()
      ? ctx.playbookText.trim()
  ...
  const lines = [
    ...
    "",
    "=== MCS PLAYBOOK — APPROVED GUIDANCE ===",
    "Owner-validated business guidance only. Does not override CRM facts or deterministic CRM Next Action.",
    "Do not treat Playbook text as permission to mutate CRM.",
    playbookText,
    "",
