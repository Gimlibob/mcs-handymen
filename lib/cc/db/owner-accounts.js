...
/**
 * Atomically replace password hash and bump password_version.
 * @param {{ ownerId: string, passwordHash: string, expectedVersion?: number }} args
 * @returns {{ ok: true, owner: object } | { ok: false, error: string }}
 */
export async function updateOwnerPasswordHash({
  ...
  try {
    ...
    console.error("[cc/owner] updateOwnerPasswordHash failed");
