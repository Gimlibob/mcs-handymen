...
/**
 * Normalize month query param (YYYY-MM or YYYY-MM-DD) to YYYY-MM-01.
 * @param {unknown} value
 * @returns {string | null}
 */
export function normalizeMonthParam(value) {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
  if (/^\d{4}-\d{2}$/.test(trimmed)) {
    const candidate = `${trimmed}-01`;
    return isValidCalendarDateString(candidate) ? candidate : null;
  }
...
/**
 * First day of calendar month containing ymd (or Chicago today).
 * @param {string | null} [anchorYmd]
 */
export function monthStartContaining(anchorYmd = null) {
  const anchor =
    anchorYmd && isValidCalendarDateString(anchorYmd) ? anchorYmd : chicagoToday();
  return `${anchor.slice(0, 7)}-01`;
}

/**
 * Last civil day of the calendar month for YYYY-MM-01 (or any day in month).
 * @param {string} ymd
 */
export function monthEndContaining(ymd) {
  const start = monthStartContaining(ymd);
  if (!start) return null;
  const [y, m] = start.split("-").map(Number);
  // Day 0 of next month = last day of this month (UTC civil arithmetic)
...
/**
 * Shift a month by deltaMonths; returns YYYY-MM-01.
 * @param {string} ymd any day in the source month
 * @param {number} deltaMonths
 */
export function addCalendarMonths(ymd, deltaMonths) {
  const start = monthStartContaining(ymd);
  if (!start || !Number.isFinite(deltaMonths)) return null;
  const [y, m] = start.split("-").map(Number);
...
/**
 * Monday-first month grid containing the given civil date (or Chicago today).
 * @param {string | null} [anchorYmd]
 * @returns {{
 *   year: number,
 *   month: number,
 ...
 * }}
 */
export function monthGridContaining(anchorYmd = null) {
  const monthStart = monthStartContaining(anchorYmd);
  const monthEnd = monthEndContaining(monthStart);
  const [y, m] = monthStart.split("-").map(Number);
  const weekOfStart = mondayWeekContaining(monthStart);
  const weekOfEnd = mondayWeekContaining(monthEnd);
  const gridStart = weekOfStart.weekStart;
  const gridEnd = weekOfEnd.weekEnd;
  const days = [];
...
/**
 * Format YYYY-MM for month heading (e.g. "September 2026").
 * @param {string} monthStartYmd
 */
export function formatMonthHeading(monthStartYmd) {
  const start = monthStartContaining(monthStartYmd);
  if (!start) return "—";
  const [y, m] = start.split("-").map(Number);
