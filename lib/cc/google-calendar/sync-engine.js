...
import { getGoogleCalendarProvider } from "./get-provider.js";
...
export async function syncScheduledJobToGoogle(jobId, opts = {}) {
  try {
    ...
    const provider = opts.provider || getGoogleCalendarProvider();
    ...
    try {
      ...
      return { ok: false, error: sanitizeError(err), syncStatus: status };
    }
  } catch (err) {
    // Absolute last resort — never throw to MCS callers
    console.error("[google-calendar] syncScheduledJobToGoogle", err);
...
export async function deleteGoogleEventForJob(jobId, opts = {}) {
  try {
    ...
    console.error("[google-calendar] deleteGoogleEventForJob", err);
    return { ok: false, error: sanitizeError(err) };
  }
}
